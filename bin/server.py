...
class Server:
    ...
    def same_attributes(self, *args):
        """基于相同属性的推荐电影"""
        # 获得电影标签
        res = self.db.same_attr_history(args)
        if res:
            return [self.film_info(i) for i in res]
        self.log.info("开始电影同标签推荐")
        args_info = [self.film_info(i) for i in args]
        tags = []
        [tags.extend(i.get("tags")) for i in args_info]
        self.log.info("已获得{}部电影".format(len(args_info)))
        # 查询有相同标签的电影
        file_list = []
        for n, i in enumerate(tags):
            file_list.extend(self.find_films_by_tag(i))
            self.log.info("已获取标签 {} 数据".format(i), lev1=n / len(tags))
        ...
        self.db.save_same_attr_history(args, res_data)
        return res_data

    def same_taste(self, *args):
        """基于相同品味的推荐电影"""
        # 查找历史记录
        res = self.db.same_taste_history(args)
        if res:
            return [self.film_info(i) for i in res]
        user_list = []
        for film_id in args:
            a = self.film_reviews_list(film_id)['reviews']
            user_list += a
        # 去重
        ...
        # 获得这类人喜欢的电影
        user_data = []
        for film_id in user_list:
            user = self.user_info(film_id)
            user_data += user["films"]
            self.log.info(msg="爬取数据进度 {}%".format(int(user_list.index(film_id) / len(user_list) * 100)))
        # 去掉参数电影
        ...
        self.db.save_same_taste_history(args, res_data)
        return res_data

    def film_info(self, id, tag=None):
        """获得电影信息"""
        if tag:
            data = self.db.film_info_by_id(id) or self.crawler.film_info_by_id(id)
            if tag not in data.get("tags"):
                data["tags"].append(tag)
                return self.db.save_file(data)
        return self.db.film_info_by_id(id) or self.db.save_file(self.crawler.film_info_by_id(id))
